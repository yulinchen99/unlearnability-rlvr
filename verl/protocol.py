...
class DataProto:
    ...
    def __setitem__(self, item, value):
        """
        Set a specific item or items in the DataProto.
        ...
        Args:
            item: Can be one of:
                ...
                - torch.Tensor: A tensor of indices
            value: Can be:
                - DataProtoItem: For single integer indexing
                - DataProto: For slice, list, or array indexing
                - dict: Dictionary with 'batch' and 'non_tensor_batch' keys
        """
        if isinstance(item, int | np.integer):
            # Single index - expect DataProtoItem or dict
            if isinstance(value, DataProtoItem):
                self._set_single_item(item, value)
            elif isinstance(value, dict) and 'batch' in value and 'non_tensor_batch' in value:
                # Convert dict to DataProtoItem
                proto_item = DataProtoItem(
                    batch=value['batch'], 
                    non_tensor_batch=value['non_tensor_batch'], 
                    meta_info=value.get('meta_info', self.meta_info)
                )
                self._set_single_item(item, proto_item)
            else:
                raise TypeError(f"Expected DataProtoItem or dict for single index assignment, got {type(value)}")
        
        elif isinstance(item, slice):
            # Slice assignment - expect DataProto
            if isinstance(value, DataProto):
                self._set_slice(item, value)
            else:
                raise TypeError(f"Expected DataProto for slice assignment, got {type(value)}")
        
        elif isinstance(item, (list, np.ndarray, torch.Tensor)):
            # List/array assignment - expect DataProto
            if isinstance(value, DataProto):
                self._set_indices(item, value)
            else:
                raise TypeError(f"Expected DataProto for list/array assignment, got {type(value)}")
        
        else:
            raise TypeError(f"Indexing with {type(item)} is not supported for assignment")

    def _set_single_item(self, index: int, value: 'DataProtoItem'):
        """Set a single item using DataProtoItem."""
        # Validate index bounds
        if index < 0 or index >= len(self):
            raise IndexError(f"Index {index} is out of bounds for DataProto of length {len(self)}")
        
        # Update batch tensors
        if self.batch is not None and value.batch is not None:
            for key in value.batch.keys():
                if key in self.batch.keys():
                    self.batch[key][index] = value.batch[key]
                else:
                    raise KeyError(f"Key '{key}' not found in target batch")
        
        # Update non_tensor_batch arrays
        for key in value.non_tensor_batch.keys():
            if key in self.non_tensor_batch.keys():
                self.non_tensor_batch[key][index] = value.non_tensor_batch[key]
            else:
                raise KeyError(f"Key '{key}' not found in target non_tensor_batch")

    def _set_slice(self, slice_obj: slice, value: 'DataProto'):
        """Set a slice of items using DataProto."""
        # Get the slice range
        ...
        if len(value) != slice_length:
            raise ValueError(f"Slice length {slice_length} does not match value length {len(value)}")
        
        # Update batch tensors
        if self.batch is not None and value.batch is not None:
            for key in value.batch.keys():
                if key in self.batch.keys():
                    self.batch[key][slice_obj] = value.batch[key]
                else:
                    raise KeyError(f"Key '{key}' not found in target batch")
        ...
        for key in value.non_tensor_batch.keys():
            if key in self.non_tensor_batch.keys():
                self.non_tensor_batch[key][slice_obj] = value.non_tensor_batch[key]
            else:
                raise KeyError(f"Key '{key}' not found in target non_tensor_batch")

    def _set_indices(self, indices, value: 'DataProto'):
        """Set items at specific indices using DataProto."""
        # Convert indices to numpy array for consistency
        ...
        if len(value) != len(indices):
            raise ValueError(f"Number of indices {len(indices)} does not match value length {len(value)}")
        
        # Update batch tensors
        if self.batch is not None and value.batch is not None:
            for key in value.batch.keys():
                if key in self.batch.keys():
                    self.batch[key][indices] = value.batch[key]
                else:
                    raise KeyError(f"Key '{key}' not found in target batch")
        ...
        for key in value.non_tensor_batch.keys():
            if key in self.non_tensor_batch.keys():
                self.non_tensor_batch[key][indices] = value.non_tensor_batch[key]
            else:
                raise KeyError(f"Key '{key}' not found in target non_tensor_batch")
    ...
    def check_consistency(self):
        ...
        if self.batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                assert isinstance(
                    val, np.ndarray
                ) and val.dtype == object, f'data in the non_tensor_batch must be a numpy.array with dtype=object, but got {type(val)} for key {key}'
                assert val.shape[
    ...
    def repeat(self, repeat_times=2, interleave=True):
        ...
        if self.batch is not None:
            ...
            repeated_batch = TensorDict(
                ...
                batch_size=(self.batch.batch_size[0] * repeat_times,) if isinstance(repeat_times, int) else ((repeat_times).sum().item(),)
            )
        else:
            repeated_batch = None
