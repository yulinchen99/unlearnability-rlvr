...
def parse_and_verify(solution_str, ground_truth):
    """Parses and verifies the solution string."""
    extract_answer, is_boxed_matched = extract_solution(solution_str=solution_str)
    target = parse(extract_answer)
    gold = parse(ground_truth)
    return verify(gold=gold, target=target), is_boxed_matched, extract_answer
...
def compute_score(solution_str, ground_truth):
    ...
    """
    try:
        correct, is_boxed_matched, extract_answer = global_executor.submit(parse_and_verify, solution_str, ground_truth)
    except Exception as e:
        print(f"Exception occurred: {str(e)}")
        return 0.0
