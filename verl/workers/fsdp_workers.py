...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # check if rollout.oversample_n is not -1, if so, set rollout.n to oversample_n
        # self.rollout_n = self.config.rollout.n
        if hasattr(self.config.rollout, 'oversample_n') and self.config.rollout.oversample_n != -1:
            self.config.rollout.n = self.config.rollout.oversample_n
            print(f"rollout.n is set to {self.config.rollout.n} because rollout.oversample_n is not -1")
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            self.actor.init_entropy_controller()
    ...
    def update_actor(self, data: DataProto):
        ...
        # print("input size before sharding:", data.batch["input_ids"].size())
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            # print("input size after sharding:", data.batch["input_ids"].size())
            # perform training
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # print cpu memory
            # import psutil
            # print("cpu memory before compute_log_prob:", psutil.virtual_memory().used / (1024 * 1024 * 1024), "GB")

            log_probs, last_token_hidden_states = self.actor.compute_log_prob(data=data)

            # print("cpu memory after compute_log_prob:", psutil.virtual_memory().used / (1024 * 1024 * 1024), "GB")

            output = DataProto.from_dict(tensors={'old_log_probs': log_probs,
        ...
        # print("cpu memory after put logprob output to cpu:", psutil.virtual_memory().used / (1024 * 1024 * 1024), "GB")
        ...
        # print("cpu memory at the end of compute_log_prob function:", psutil.virtual_memory().used / (1024 * 1024 * 1024), "GB")
        return output
