...
def remove_boxed(string):
    # extract answerfrom \\boxed{}
    string = string.strip().strip("$").strip()
    assert string.startswith("\\\\boxed{"),print("string:", string)
    assert string.endswith("}"),print("string:", string)
    answer = string[len("\\\\boxed{"):-1]
    # assert answer is not None
    # answer = answer.group(1)
    # print("extract answer from", string, "is:", answer)
    return answer
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    # parse ground truth
    if data_name in ["math", "minerva_math", "math_500", "math_level_1", "math_level_3_5", "math_level_1_4", "math_train"]:
        gt_cot = example["solution"]
        gt_ans = extract_answer(gt_cot, data_name)
    elif data_name == "math-squared":
        gt_cot = example["solution"]
        gt_ans = remove_boxed(example["final_answer"])
    elif data_name in ["math_perturb_simple", "math_perturb_hard", "simplelr_qwen_level1to4_augment_unlearnable", "simplelr_qwen_level1to4_augment_2k_except_unlearnable"] or "numina" in data_name:
        ...
        gt_cot = example["target"]
        gt_ans = example["extra_info"]["answer"]
    elif "reward_model" in example:
        gt_cot = None
        gt_ans = example["reward_model"]["ground_truth"]
    else:
...
def parse_question(example, data_name):
    # print("parsing question for:", data_name)
    question = ""
    if data_name == "asdiv":
    ...
    if data_name in ["gpqa_diamond", "webinstruct_rlpr"]:
        question += "\nFinal answer should be one of the following options: (A), (B), (C), (D), etc."
    elif "mmlupro" in data_name:
        question += "\nFinal answer should be one of the following options: (A), (B), (C), (D), (E), (F), (G), (H), (I), (J), etc."
